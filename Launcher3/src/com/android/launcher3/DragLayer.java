...
20160506     liyichong.wt   bug 173387          requirements        modify search view bug
20160708     liyichong.wt   bug 196245          bugs                modify bug
20160720     liyichong.wt   bug 200032          bugs                modify bug
******************************************************************************************************/
/**
...
public class DragLayer extends FrameLayout implements ViewGroup.OnHierarchyChangeListener {
    ...
    private static final float SCREEN_MANAGER_THRESHOLD = 0.95f;
    private static final float FREQUENT_USED_APPS_THRESHOLD = 1.05f;

    /* bug 155071, liyichong.wt, ADD, 20160420 start*/
    private int mHotseatHeight = 252;
    private int mDefaultScreenHeight = 1920;
    private int mDragWidthSimilarlyIconSize = 174;//bug 196245, liyichong.wt, ADD, 20160708
    final int RIGHT = 0;
    final int LEFT = 1;
    ...
    private GestureDetector.OnGestureListener onGestureListener = new GestureDetector.SimpleOnGestureListener() {
        ...
        public boolean onFling(MotionEvent e1, MotionEvent e2, float velocityX,
            ...
            if (workspace == null
                    || workspace.isInOverviewMode()
                    || workspace.isInIconManageOverviewMode()
                    || workspace.isSwitchingState()
                    || workspace.isOnOrMovingToCustomContent()//bug 200032, liyichong.wt, ADD, 20160720
                    || !mLauncher.isWorkspaceState()
                    || mLauncher.isFrequentUsedAppsShowing()
                    || mLauncher.isCmccSearchAppsShowing()//bug 173387, liyichong.wt, ADD, 20160506
                    || mLauncher.hasOpenedFolder()
                    || mLauncher.isWorkspaceLocked()
                    || mLauncher.isPaused()) {
                return false;
            }

            if(Math.min(e1.getY(), e2.getY()) < (mDefaultScreenHeight - mHotseatHeight)){
                return false;
            }

            if (x > mDragWidthSimilarlyIconSize) {//bug 196245, liyichong.wt, ADD, 20160708
                doResult(RIGHT);
            } else if (x < 0) {
                doResult(LEFT);
            }
    ...
    private boolean mGestureDetectorEatActionUp = true;
    ...
    private OnScaleGestureListener mGestureListener = new OnScaleGestureListener() {
        ...
        public boolean onScale(ScaleGestureDetector detector) {
            ...
            if (scale <= SCREEN_MANAGER_THRESHOLD
                    // mDropView is not null indicate that drop animation not end
                    // if we enter overview mode will cause status bar visible.
                    && mDropView == null
                    // Hotseat's alpha will be 1.0f if getChangeStateAnimation(...)
                    // called before startUnlockAnimation(...), this is because
                    // the first animator of Hotseat will stop.
                    // So if startUnlockAnimation(...) calls later, it will cause
                    // Hotseat alpha change to 1.0f in screen manage mode.
                    && workspace.isUnlockAnimationStarted()) {
                mLauncher.getWorkspace().enterOverviewMode();
                return true;
            }

            if (scale >= FREQUENT_USED_APPS_THRESHOLD) {
                mLauncher.showFrequentUsedApps();
                return true;
            }
    ...
    public DragLayer(Context context, AttributeSet attrs) {
        ...
        mGestureDetector = new ScaleGestureDetector(context, mGestureListener);

        gestureDetector = new GestureDetector(context,onGestureListener);//bug 155071, liyichong.wt, ADD, 20160420
    ...
    private boolean isEventOverFolder(Folder folder, MotionEvent ev) {
        if (folder.isFullScreen()) {
            //tangzhongfeng.wt add 20160420, when in icon manage state
            //bug 204685 tangzhongfeng.wt MODIFY 20160803
            if (LauncherAppState.getInstance().getLauncherInEdit()) {
                if (ev.getAction() != MotionEvent.ACTION_DOWN
                        || folder.getIconManageRect().contains((int) ev.getX(),
                                (int) ev.getY())) {
                    return true;
                }
            }
            //bug 197656 tangzhongfeng.wt ADD 20160713
            if (folder.getAllContentRect().top > (int) ev.getY()) {
                return true;
            }
            // for click empty position to close folder
            mHitRect.set(folder.getAllContentRect());
        } else {
    ...
    private boolean handleTouchDown(MotionEvent ev, boolean intercept) {
        ...
        if (currentFolder != null && intercept) {
            ...
            if (!isEventOverFolder(currentFolder, ev)) {
                //bug 181064 tangzhongfeng.wt ADD 20160526 
                mLauncher.mIsCloseForKeyback = true;
    ...
    public boolean onInterceptTouchEvent(MotionEvent ev) {
        ...
        if (action == MotionEvent.ACTION_DOWN) {
            if (!mGestureDetectorEatActionUp) {
                // mimic a ACTION_UP for gestrue
                MotionEvent mimic = MotionEvent.obtain(ev);
                mimic.setAction(MotionEvent.ACTION_UP);
                mGestureDetector.onTouchEvent(mimic);
                gestureDetector.onTouchEvent(mimic);//bug 155071, liyichong.wt, ADD, 20160420
                mimic.recycle();
            }
            mGestureDetectorEatActionUp = false;
        }
        mGestureDetector.onTouchEvent(ev);
        gestureDetector.onTouchEvent(ev);//bug 155071, liyichong.wt, ADD, 20160420
        ...
        if (action == MotionEvent.ACTION_DOWN) {
            ...
            if (action == MotionEvent.ACTION_UP) {
                mGestureDetectorEatActionUp = true;
            }
        ...
        // in order to detect the scale gesture, we intercept the touch event
        // when user touch screen with more than one finger.
        if (ev.getPointerCount() >= 2) {
            if (mLauncher != null && mLauncher.getWorkspace() != null) {
                mLauncher.getWorkspace().setAllowLongPress(false);
            }
            return true;
        }
    ...
    public boolean onTouchEvent(MotionEvent ev) {
        ...
        if (action == MotionEvent.ACTION_DOWN) {
            ...
            mTouchCompleteListener = null;
            if (mLauncher != null && mLauncher.getWorkspace() != null) {
                mLauncher.getWorkspace().setAllowLongPress(true);
            }
    ...
    public void animateViewIntoPosition(DragView dragView, final View child, int duration,
            final Runnable onFinishAnimationRunnable, View anchorView) {
        // Very strange that child will be null in some cases, do something here to avoid NPE.
        if (child == null) {
            return;
        }
     ...
     */
    /* bug 155015, liyichong.wt, DELETE, 20160405 start*/
    /*private boolean isLayoutRtl() {
        return (getLayoutDirection() == LAYOUT_DIRECTION_RTL);
    }*/
    /* bug 155015, liyichong.wt, DELETE, 20160405 end*/
    ...
    private void drawPageHints(Canvas canvas) {
        if (mShowPageHints) {
            ...
            final boolean isRtl = dragLayer.getLayoutDirection() == View.LAYOUT_DIRECTION_RTL;//isLayoutRtl()
     ...
     * @return
     */
    private String strMotionEventAction(int action) {
        switch (action) {
