...
public class DeleteDropTarget extends ButtonDropTarget {
    ...
    private AnimationDrawable mRemoveTrashcanDrawable;
    private AnimationDrawable mRemoveTrashcanDrawableReverse;
    private Drawable mRemoveTranshcanDrawableStart;

    private boolean mWaitingForUninstall = false;

    private boolean mUseUninstallLabel;
    ...
    protected void onFinishInflate() {
        ...
        mRemoveTrashcanDrawable = (AnimationDrawable) r.getDrawable(R.drawable.trashcan);
        mRemoveTrashcanDrawableReverse = (AnimationDrawable) r.getDrawable(R.drawable.trashcan_reverse);
        mRemoveTranshcanDrawableStart = r.getDrawable(R.drawable.ic_drop_delete_frame1);
    ...
    private boolean isDragSourceWorkspaceOrFolder(DragObject d) {
        return (d.dragSource instanceof Workspace) || (d.dragSource instanceof Folder) || (d.dragSource instanceof CmccN2IconManagePanel);
    }
    private boolean isWorkspaceOrFolderApplication(DragObject d) {
        //Bug 163862 tangzhongfeng.wt MODIFY 20160426
        return isDragSourceWorkspaceOrFolder(d) && (d.dragInfo instanceof ShortcutInfo && ((ShortcutInfo) d.dragInfo).itemType == LauncherSettings.Favorites.ITEM_TYPE_APPLICATION);
    }
    //Bug 163862 tangzhongfeng.wt MODIFY 20160426
    private boolean isWorkspaceOrFolderShortcut(DragObject d) {
        return isDragSourceWorkspaceOrFolder(d) && (d.dragInfo instanceof ShortcutInfo && ((ShortcutInfo) d.dragInfo).itemType == LauncherSettings.Favorites.ITEM_TYPE_SHORTCUT);
    }
    ...
    private void setHoverColor() {
        if (mCurrentDrawable != null && mCurrentDrawable instanceof TransitionDrawable) {
            ((TransitionDrawable) mCurrentDrawable).startTransition(mTransitionDuration);
        }
        setTextColor(mHoverColor);
    }
    private void resetHoverColor() {
        if (mCurrentDrawable != null && mCurrentDrawable instanceof TransitionDrawable) {
            ((TransitionDrawable) mCurrentDrawable).resetTransition();
        }
    ...
    public void onDragStart(DragSource source, Object info, int dragAction) {
        ...
        //Bug 163862 tangzhongfeng.wt MODIFY 20160425
        boolean useUninstallLabel = isAllAppsApplication(source, info)
                || (LauncherAppState.isDisableAllApps() && (info instanceof ShortcutInfo && ((ShortcutInfo) info).itemType == LauncherSettings.Favorites.ITEM_TYPE_APPLICATION));
        boolean useDeleteLabel = !useUninstallLabel && source.supportsDeleteDropTarget();

        mUseUninstallLabel = useUninstallLabel;

        // If we are dragging an application from AppsCustomize, only show the control if we can
        // delete the app (it was downloaded), and rename the string to "uninstall" in such a case.
        // Hide the delete target if it is a widget from AppsCustomize.
        if (!willAcceptDrop(info) || isAllAppsWidget(source, info) || mLauncher.getWorkspace().isInIconManageOverviewMode()) {
            isVisible = false;
        }
        ...
        if (useUninstallLabel) {
            ...
            //setCompoundDrawablesRelativeWithIntrinsicBounds(mRemoveTranshcanDrawableStart, null, null, null);
        ...
        if (isVisible && getText().length() > 0) {
            int deleteLabel = (source instanceof Folder) ? R.string.folder_drag_delete_hint
    ...
    public void onDragEnter(DragObject d) {
        ...
        /*if (mCurrentDrawable == mRemoveTranshcanDrawableStart
                || mCurrentDrawable == mRemoveTrashcanDrawableReverse) {
            mRemoveTrashcanDrawable.stop();
            mRemoveTrashcanDrawableReverse.stop();
            setCompoundDrawablesRelativeWithIntrinsicBounds(
                    mRemoveTrashcanDrawable, null, null, null);
            mCurrentDrawable = getCurrentDrawable();
            mRemoveTrashcanDrawable.start();
        }*/
    ...
    public void onDragExit(DragObject d) {
        ...
        /*if (mCurrentDrawable == mRemoveTrashcanDrawable) {
            setCompoundDrawablesRelativeWithIntrinsicBounds(
                    mRemoveTrashcanDrawableReverse, null, null, null);
            mCurrentDrawable = getCurrentDrawable();
            mRemoveTrashcanDrawableReverse.start();
        }*/
    }
    ...
    private void completeDrop(DragObject d) {
        ...
        } else if (isWorkspaceOrFolderApplication(d) || isWorkspaceOrFolderShortcut(d)) { //Bug 163862 tangzhongfeng.wt MODIFY 20160426
    ...
    public void setDeleteHintWhenDragging(int resId) {
        if (!mUseUninstallLabel && mActive) {
            setText(resId);
        }
    }
}
