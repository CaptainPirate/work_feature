...
|*************************************************************************************|
| 20160331|  liyichong.wt|   Req_155026  |    requirements    |add 4x1 personal widget|
**************************************************************************************/
...
public class CellLayout extends ViewGroup {
    ...
    private boolean mIsIconManage = false;
    private float mIconManageScale = 1f;
    ...
    private boolean mHasTopBar;
    private boolean mTakeScaleCompensation;
    private View mTopBar;
    private Rect mTopBarHitRect;
    private TextView mSetDefaultPage;
    private TextView mSelfDelete;
    private ImageView mAddImageView;

    private static int sTopBarHeight;
    private static float sScaleCompensation = 1.0f;
    ...
    public CellLayout(Context context, AttributeSet attrs, int defStyle) {
        ...
        mMaxGap = a.getDimensionPixelSize(R.styleable.CellLayout_maxGap, Integer.MAX_VALUE);
        ...
        /* REQ-Launcher-01.01.08, tangzhongfeng.wt, MODIFY, 20160401 end*/
        ...
        mHasTopBar = a.getBoolean(R.styleable.CellLayout_hasTopBar, false);
        mTakeScaleCompensation = a.getBoolean(R.styleable.CellLayout_takeScaleCompensation, false);
        ...
        mIconManageScale = (float) grid.editTempBgWidth / grid.iconSizePx;
        Log.i(TAG, "tzf mIconManageScale = " + mIconManageScale);
        ...
        // add top bar
        if (mHasTopBar) {
            mTopBar = mLauncher.getInflater().inflate(R.layout.celllayout_top_bar, null);
            mTopBarHitRect = new Rect();
            addView(mTopBar);

            mSetDefaultPage = (TextView) mTopBar.findViewById(R.id.top_bar_set_home);
            mSetDefaultPage.setOnClickListener(new OnClickListener() {
                @Override
                public void onClick(View v) {
                    setAsUserDefaultHomePage();
                }
            });

            mSelfDelete = (TextView) mTopBar.findViewById(R.id.top_bar_del_page);
            mSelfDelete.setOnClickListener(new OnClickListener() {
                @Override
                public void onClick(View v) {
                    deleteSelfPage();
                }
            ...
            updateUserDefaultPageButton(-1, -2);
            updateSelfDeleteButton(-1, -2);
        }
    }
    ...
    public float getChildrenScale() {
        ...
        } else if (mIsIconManage) {
            return mIconManageScale;
        }
        return 1.0f;
    }
    ...
    void setIsDragOverlapping(boolean isDragOverlapping) {
        if (isInAddMode()) {
            mAddImageView.setPressed(isDragOverlapping);
            return;
        }
    ...
    public void setIsIconManage(boolean isIconManage) {
        mIsIconManage = isIconManage;
        mShortcutsAndWidgets.setIsIconManage(isIconManage);
    }
    ...
    public boolean addViewToCellLayout(View child, int index, int childId, LayoutParams params,
        ...
        if (child instanceof BubbleTextView) {
            ...
            boolean textDisabled = mLauncher.getHotseat() != null ? mLauncher.getHotseat()
                    .isHotseatDisabled() : false;
            //REQ-Launcher-01.01.08, tangzhongfeng.wt, MODIFY, 20160401
            bubbleChild.setTextVisibility(!(mIsHotseat && textDisabled || mIsIconManage));
            Log.i(TAG,"-----204667-----Title = "+bubbleChild.getText());
        }
    ...
    public boolean onInterceptTouchEvent(MotionEvent ev) {
        ...
        if (mHasTopBar) {
            mTopBar.getHitRect(mTopBarHitRect);
            if (mTopBarHitRect.contains((int) ev.getX(), (int) ev.getY())) {
                return false;
            }
        }
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if (mTakeScaleCompensation) {
            childHeightSize = Math.round((childHeightSize * sScaleCompensation));
        }
        ...
        for (int i = 0; i < count; i++) {
            ...
            int childH = newHeight;
            if (child == mTopBar) {
                newHeight = sTopBarHeight;
            } else if (child == mAddImageView
            ...
            // restore available child height for next child
            newHeight = childH;
        }
        if (mFixedWidth > 0 && mFixedHeight > 0) {
    ...
    protected void onLayout(boolean changed, int l, int t, int r, int b) {
        ...
        for (int i = 0; i < count; i++) {
            top = getPaddingTop();
            View child = getChildAt(i);
            if (child == mShortcutsAndWidgets) {
                int shortcutH = mShortcutsAndWidgets.getMeasuredHeight();
                if (getParent() instanceof Workspace) {
                    // in workspace the shortcut container's height equals to
                    // the height of CellLayout in normal mode, but in overview
                    // mode, it's height is 90% of the CellLayout's height. 90%
                    // is come from the design UI picture.
                    top += getMeasuredHeight() - shortcutH;
                }
                child.layout(left, top, left + r - l, top + shortcutH);
            } else if (child == mTopBar) {
            	/*tangzhongfeng.wt, ADD, 20160406 set mSelfDelete GONE when the screen has child  start */
            	if (mShortcutsAndWidgets.getChildCount() == 0) {
            		mSelfDelete.setVisibility(View.VISIBLE);
            	} else {
            		mSelfDelete.setVisibility(View.GONE);
            	}
            	/*tangzhongfeng.wt, ADD, 20160406 set mSelfDelete GONE when the screen has child  end */
                int shortcutH = mShortcutsAndWidgets.getMeasuredHeight();
                top += getMeasuredHeight() - shortcutH - sTopBarHeight;
                child.layout(left, top, left + r - l, top + sTopBarHeight);
            } else if (child == mAddImageView
                    && child.getVisibility() == View.VISIBLE) {
                ...
                int cL = ((r - l) - mW) / 2;
                int cT = ((b - t) - mH) / 2;
                child.layout(cL, cT, cL + mW, cT + mH);
            } else {
    ...
    public boolean animateChildToPosition(final View child, int cellX, int cellY, int duration,
        ...
        if (clc.indexOfChild(child) != -1) {
            // Very strange that ArrayIndexOutOfBoundsException will happen here.
            // Add protected code to avoid this temporary.
            if (cellX >= occupied.length) {
                return false;
            } else {
                if (cellY >= occupied[cellX].length) {
                    return false;
                }
            }
    ...
    private void animateItemsToSolution(ItemConfiguration solution, View dragView, boolean
        ...
        for (int i = 0; i < childCount; i++) {
            ...
            if (c != null) {
                ...
                //tangzhongfeng.wt, ADD, 20160405 skip the bg child
                boolean value = true;
                if (child instanceof ImageView) {
                    value = false;
                }
                markCellsForView(c.x, c.y, c.spanX, c.spanY, occupied, value);
            }
        }
        if (commitDragView) {
            markCellsForView(solution.dragViewX, solution.dragViewY, solution.dragViewSpanX,
        ...
        while (true) {
            ...
            for (int y = startY; y < endY && !foundCell; y++) {
                ...
                for (int x = startX; x < endX; x++) {
                    ...
                    Log.i(TAG,"-----204667-----findCellForSpanThatIntersectsIgnoring x = " + x + " y = " + y);
                    if (cellXY != null) {
                        cellXY[0] = x;
                        cellXY[1] = y;
                    }
...
out:            for (int i = x; i < x + spanX - 1 && x < xCount; i++) {
    ...
    public boolean isInAddMode() {
        return (mAddImageView != null
                && mAddImageView.getVisibility() == View.VISIBLE);
    }

    public void enableAddModeIfNeeded(boolean enable) {
        if (mAddImageView == null) {
            mAddImageView = new ImageView(mLauncher);
            mAddImageView.setImageResource(R.drawable.add_new_page_selector);
            mAddImageView.setVisibility(View.GONE);
            mAddImageView.setAdjustViewBounds(true);
            mAddImageView.setOnClickListener(new OnClickListener() {
                @Override
                public void onClick(View v) {
                    CellLayout.this.performClick();
                }
            });
            addView(mAddImageView);
        }

        if (!isInAddMode() && enable) {
            // hide top bar if needed
            if (mTopBar != null) {
                mTopBar.setVisibility(View.INVISIBLE);
            }
            mAddImageView.setVisibility(View.VISIBLE);
        } else if (isInAddMode() && !enable) {
            // show top bar if needed
            if (mTopBar != null) {
    ...
    private void setAsUserDefaultHomePage() {
        Workspace parent = mLauncher.getWorkspace();
        int meIndex = parent.getChildIndexIgnoreExtraCellLayout(this);
        parent.setUserDefaultPageIndex(meIndex);
        parent.updateCellLayoutTopBar();
    }

    private void deleteSelfPage() {
        final Workspace parent = mLauncher.getWorkspace();
        final int nextUserDefaultPage = parent
                .getNextUserDefaultIndexIfDeletePage(this);
        if (nextUserDefaultPage == -1) {
            // prompt the last page can not be deleted
            Toast.makeText(mLauncher, R.string.delete_last_page_hint,
                    Toast.LENGTH_SHORT).show();
            return;
        }

        // if has no item, just delete this page
        if (mShortcutsAndWidgets.getChildCount() == 0) {
            parent.deletePage(this, nextUserDefaultPage);
            return;
        }

        // else alert user to confirm delete
        final AlertDialog dialog = new AlertDialog.Builder(mLauncher, AlertDialog.THEME_DEVICE_DEFAULT_LIGHT)
                .setTitle(R.string.delete_page_dialog_title)
                .setPositiveButton(R.string.delete_page_dialog_button,
                        new DialogInterface.OnClickListener() {
                            public void onClick(DialogInterface dialog,
                                    int whichButton) {
                                parent.deletePage(CellLayout.this, nextUserDefaultPage);
                            }
                        })
                .setNegativeButton(R.string.alert_dialog_cancel, null)
                .create();

        // Dismiss the dialog when user press Home key or Recent Apps key
        mLauncher.setDeletePageDialo(dialog);

        String msg = mLauncher.getResources().getString(R.string.delete_page_dialog_msg);
        dialog.setMessage(msg);
        dialog.show();
        dialog.getButton(AlertDialog.BUTTON_POSITIVE).setTextColor(Color.RED);
    }

    public void updateUserDefaultPageButton(int meIndex, int defaultIndex) {
        if (isInAddMode()) {
            return;
        }

        int drawableId = 0;
        if (meIndex == defaultIndex) {
            // already user default page
            drawableId = R.drawable.user_default_page;
        } else {
            // can set as user default page
            drawableId = R.drawable.user_default_page_set;
        }
        mSetDefaultPage.setCompoundDrawablesWithIntrinsicBounds(drawableId, 0,
                0, 0);
    }

    public void updateSelfDeleteButton(int meIndex, int defaultIndex) {
        if (isInAddMode()) {
            return;
        }
        mSelfDelete.setCompoundDrawablesWithIntrinsicBounds(
                R.drawable.page_delete_enable, 0, 0, 0);
    }
    ...
    public static void setScaleCompensation(float value) {
        sScaleCompensation = value;
    }

    public static float getScaleCompensatoin() {
        return sScaleCompensation;
    }

    public static void setTopBarHeight(int height) {
        sTopBarHeight = height;
    }
    ...
    /**
     * You should use fastGetPersonalCenterWidget() if it fit your requirement.
     * @return
     */
    public View getPersonalCenterWidget() {
        int itemCount = mShortcutsAndWidgets.getChildCount();
        for (int i=0; i<itemCount; i++) {
            ...
            if (info != null && info.itemType == LauncherSettings.Favorites.ITEM_TYPE_APPWIDGET) {
                ...
                ComponentName cn = widget.providerName;
                /* Req_155026, liyichong.wt, MODIFY, 20160331 start */
                if (AppsCustomizePagedView.sPersonalCenter.equals(cn) || AppsCustomizePagedView.sPersonalCenter4x1.equals(cn)) {
                /* Req_155026, liyichong.wt, MODIFY, 20160331 end */
                    return child;
                }
            }
        }
    ...
    public View fastGetPersonalCenterWidget() {
        View child = getChildAt(0, 0);
        if (child != null) {
            ItemInfo info = (ItemInfo) child.getTag();
            if (info != null && info.itemType == LauncherSettings.Favorites.ITEM_TYPE_APPWIDGET) {
                LauncherAppWidgetInfo widget = (LauncherAppWidgetInfo) info;
    ...
    public int findPosition(int[] vacant, int max_cellX, int max_cellY, boolean isFromLast) {
        int x = 0;
        int y = 0;
        int retCount = 0;
        ...
        vacant[1] = y;
        if (isFromLast) {
            for (y = max_cellY - 1; y >= 0; y--) {
                for (x = max_cellX - 1; x >= 0; x--) {
                    if (mOccupied[x][y]) {
                        retCount = y * max_cellX + x + 1;
                        return retCount;
                    } else {
                        vacant[0] = x;
                        vacant[1] = y;
            ...
            for (y = 0; y < max_cellY; y++) {
                for (x = 0; x < max_cellX; x++) {
                    if (mOccupied[x][y]) {
                        retCount = y * max_cellX + x + 1;
                    } else {
                        vacant[0] = x;
