...
public class InfoDropTarget extends ButtonDropTarget {
    ...
    private boolean mWaitingForDetail;
    ...
    public void onDragStart(DragSource source, Object info, int dragAction) {
        ...
        if (source.supportsAppInfoDropTarget() && (info instanceof ShortcutInfo && ((ShortcutInfo) info).itemType == LauncherSettings.Favorites.ITEM_TYPE_APPLICATION)) {
            isVisible = true;
        }
    ...
    private void completeDrop(final DragObject d) {
    	deferCompleteDropIfShowingDetail(d);
    	final DragSource dragSource = d.dragSource;
        final Runnable waitForShowingDetail = new Runnable() {
            @Override
            public void run() {
                if (dragSource instanceof Folder) {
                    ((Folder) dragSource).
                    onShowDetailActivityReturned(true);
        ...
        mLauncher.addOnResumeCallback(waitForShowingDetail);
    }
    private void deferCompleteDropIfShowingDetail(DragObject d) {
        if (d.dragSource instanceof Folder) {
            ((Folder) d.dragSource).deferCompleteDropAfterShowDetailActivity();
        } else if (d.dragSource instanceof Workspace) {
            ((Workspace) d.dragSource).deferCompleteDropAfterShowDetailActivity();
        }
    }
}
