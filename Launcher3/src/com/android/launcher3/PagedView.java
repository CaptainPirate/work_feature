...
public abstract class PagedView extends ViewGroup implements ViewGroup.OnHierarchyChangeListener {
    ...
    protected boolean mIsSupportEmptyScreen = true;
    // when support empty screen, there will have two extra page for adding new
    // page.
    protected int[] mPageScrollsForEmptyScreen;
    protected boolean mFakeOverview;
    protected int mHeightCompensation;
    protected int mWidthCompensation;
    ...
    protected boolean computeScrollHelper() {
        if (mScroller.computeScrollOffset()) {
            ...
            if (getScrollX() != mScroller.getCurrX()
                ...
                float scaleX = (mFreeScroll && !mIsSupportEmptyScreen) ? getScaleX() : 1f;
            ...
            if (child.getVisibility() != GONE) {
                ...
                if (!lp.isFullScreenPage) {
                    ...
                    childWidth = getViewportWidth() - horizontalPadding
                            - mInsets.left - mInsets.right - mWidthCompensation;
                    childHeight = getViewportHeight() - verticalPadding
                            - mInsets.top - mInsets.bottom + mHeightCompensation;
    ...
    protected void onLayout(boolean changed, int left, int top, int right, int bottom) {
        ...
        int childLeft = offsetX + (lp.isFullScreenPage ? 0 : (getPaddingLeft() + mWidthCompensation / 2));
        if (mPageScrolls == null || getChildCount() != mChildCountOnLastLayout) {
            mPageScrolls = new int[getChildCount()];
            mPageScrollsForEmptyScreen = new int[getChildCount() + 2];
        }
        ...
        for (int i = startIndex; i != endIndex; i += delta) {
            ...
            if (child.getVisibility() != View.GONE) {
                ...
                if (lp.isFullScreenPage) {
                    ...
                    childTop = offsetY + getPaddingTop() + mInsets.top - mHeightCompensation;
                    if (mCenterPagesVertically) {
                        childTop += (getViewportHeight() - mInsets.top
                                - mInsets.bottom - verticalPadding
                                - child.getMeasuredHeight() + mHeightCompensation) / 2;
                    }
                }
                ...
                int scrollOffsetLeft = lp.isFullScreenPage ? 0 : (getPaddingLeft() + mWidthCompensation / 2);
        ...
        if (mIsSupportEmptyScreen) {
            int unit4Scroll = mViewport.width() - 2 * getPaddingLeft() + mPageSpacing - mWidthCompensation;
            if (isRtl) {
                // FIXME: current don't support RTL
                for (int i = startIndex; i != endIndex; i += delta) {
                    mPageScrollsForEmptyScreen[i + 2] = mPageScrolls[i];
                }
                mPageScrollsForEmptyScreen[1] = unit4Scroll * (startIndex + 2 - 1);
                mPageScrollsForEmptyScreen[0] = unit4Scroll * (startIndex + 2);
            } else {
                for (int i = startIndex; i != endIndex; i += delta) {
                    mPageScrollsForEmptyScreen[i] = mPageScrolls[i];
                }
                mPageScrollsForEmptyScreen[endIndex] = (unit4Scroll) * endIndex;
                mPageScrollsForEmptyScreen[endIndex + 1] = (unit4Scroll) * (endIndex + 1);
            }
        }
    ...
    public void enablePageIndicatorAnimation(boolean animate) {
        if (mPageIndicator != null) {
            mPageIndicator.enableAnimate(animate);
        }
    }
    ...
    protected void getVisiblePages(int[] range) {
        ...
        if (pageCount > 0) {
            ...
            for (int i = 0; i < count; i++) {
                ...
                Utilities.getDescendantCoordRelativeToParentIgnoreTransformation(
                                currPage, this, mTmpIntPoint, false);
    ...
    public void addFocusables(ArrayList<View> views, int direction, int focusableMode) {
        ...
        if (direction == View.FOCUS_LEFT) {
            if (mCurrentPage > 0) {
                // It's very strange that mCurrentPage may large than getPageCount().
                // So it will cause NPE here when try to call addFocusables(...) on
                // a null object, we add some protective code here to avoid NPE.
                int focusIndex = Math.max(0, Math.min(mCurrentPage, getPageCount()));
                View focusPage = getPageAt(focusIndex - 1);
                if (focusPage != null) {
                    focusPage.addFocusables(views, direction, focusableMode);
                }
            }
        } else if (direction == View.FOCUS_RIGHT){
            ...
            case MotionEvent.ACTION_DOWN: {
                ...
                if (finishedScrolling) {
                    ...
                    if (!mScroller.isFinished() && !mFreeScroll) {
                        ...
                        // We should manualy call onPostReorderingAnimationCompleted()
                        // to resume page size if we are reordering. For setCurrentPage(...)
                        // will cause scroll stop and onPostReorderingAnimationCompleted()
                        // won't be called in computeScrollHelper().
                        if (isReordering(false)) {
                            onPostReorderingAnimationCompleted();
                        }
                    }
    ...
    public int getScrollForPage(int index) {
        if (mIsSupportEmptyScreen && mPageScrollsForEmptyScreen != null
                && mFakeOverview) {
            // in monkey case, index may equals to mPageScrollsForEmptyScreen.lengh
            // avoid crash here.
            if (index >= mPageScrollsForEmptyScreen.length || index < 0) {
                return 0;
            }
            return mPageScrollsForEmptyScreen[index];
        }
    ...
    public boolean onTouchEvent(MotionEvent ev) {
        ...
        case MotionEvent.ACTION_MOVE:
            if (mTouchState == TOUCH_STATE_SCROLLING) {
                ...
                if (pageUnderPointIndex > -1 && pageUnderPointIndex != indexOfChild(mDragView) &&
                    ...
                    //getFreeScrollPageRange(mTempVisiblePagesRange);
                    getReorderScrollPageRange(mTempVisiblePagesRange);
        ...
        case MotionEvent.ACTION_UP:
            if (mTouchState == TOUCH_STATE_SCROLLING) {
                ...
                View currPage = getPageAt(mCurrentPage);
                // Our default page some time not init as 0, so there is chance that
                // our page not ready for touch event.
                if (currPage == null) {
                    // Early break, do more extra work here.
                ...
                final int pageWidth = currPage.getMeasuredWidth();
                ...
                if (!mFreeScroll || mIsSupportEmptyScreen) {
    ...
    public boolean startReordering(View v) {
        ...
        if (mTempVisiblePagesRange[0] <= dragViewIndex &&
            ...
            startReorderingAnimate(mDragView);
    ...
    protected void startReorderingAnimate(View DragView) {
        mDragView.animate().scaleX(1.15f).scaleY(1.15f).setDuration(100).start();
    }

    protected void updateDragViewTranslation() {
        if (isReordering(true)) {
            float[] p = mapPointFromParentToView(this, mParentDownMotionX, mParentDownMotionY);
            mLastMotionX = p[0];
            mLastMotionY = p[1];
