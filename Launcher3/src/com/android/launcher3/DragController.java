...
public class DragController {
    ...
    public DragView startDrag(Bitmap b, int dragLayerX, int dragLayerY,
        ...
        handleDragPersonalCenter(dragInfo, dragView);
    ...
    private void checkTouchMove(DropTarget dropTarget) {
        if (dropTarget != null) {
            if (mLastDropTarget != dropTarget) {
                if (mLastDropTarget != null) {
                    if (mLastDropTarget instanceof Folder
                            && dropTarget instanceof ButtonDropTarget) {
                        ((Folder) mLastDropTarget).suppressClose(true);
                    }
                    mLastDropTarget.onDragExit(mDragObject);
                }
    ...
    private void drop(float x, float y) {
        ...
        if (dropTarget != null) {
            ...
            if (folder != null && folder.isCloseSuppressed()) {
                folder.suppressClose(false);
    ...
    private void handleDragPersonalCenter(Object dragInfo, DragView dragView) {
        ComponentName cn = null;
        if (dragInfo instanceof PendingAddWidgetInfo) {
            cn = ((PendingAddWidgetInfo) dragInfo).componentName;
        } else if (dragInfo instanceof LauncherAppWidgetInfo) {
            cn = ((LauncherAppWidgetInfo) dragInfo).providerName;
        }

        if (AppsCustomizePagedView.isPersonalCenterWidget(cn)) {
            dragView.fixAtTop(true);
        }
    }
