...
public class PhotoMenu extends MenuController
    ...
    protected void initializePopup() {//初始化popmenu，比如按下setting弹出设置menu
        ...
        pref = mPreferenceGroup.findPreference(CameraSettings.KEY_CAMERA_ID);
        String cameraID = (pref != null) ? pref.getValue() : null;
        ...
        if (true){//com.huaqin.common.featureoption.FeatureOption.HQ_1590_AMAZON_CAMERA_REMOSAIC_ALGORITHM
           if (isFrontCamera){
              zsl =  mActivity.getString(R.string. pref_front_camera_zsl_default);
              setPreference(CameraSettings.KEY_ZSL, zsl); 
            }
        } 
        ///@}        
        ...
        ///M:ray add for 1590 front beauty mode {@
        if (true){//com.huaqin.common.featureoption.FeatureOption.HQ_1590_AMAZON_CAMERA_REMOSAIC_ALGORITHM
           popup1.setPreferenceEnabled(CameraSettings.KEY_ZSL, !mUI.isCountingDown() && !isFrontCamera);
        }else{
    ...
    private void updateFilterModeIcon(ListPreference scenePref, ListPreference hdrPref) {
        ...
        // add by muxudong for ICE2-1949 start
        if (!scenePref.getKey().equals(CameraSettings.KEY_SCENE_MODE)
                && !hdrPref.getKey().equals(CameraSettings.KEY_CAMERA_HDR))
            return;
        // add by muxudong for ICE2-1949 end
        if ((notSame(scenePref, CameraSettings.KEY_SCENE_MODE, Parameters.SCENE_MODE_AUTO))
            ...
            //add by muxudong for ICE2-1491 start
            buttonSetEnabled(mFlashModeSwitcher, false);
            //add by muxudong for ICE2-1491 end
            ...
            //add by muxudong for ICE2-1491 start
            buttonSetEnabled(mFlashModeSwitcher, true);
            //add by muxudong for ICE2-1491 end
        }
        ...
        if (false) {//com.huaqin.common.featureoption.FeatureOption.HQ_1520_AMAZON_CAMERA_ARC_ALGORITHM
            if (same(scenePref, CameraSettings.KEY_SCENE_MODE, CameraUtil.SCENE_MODE_HDR)
                    || same(scenePref, CameraSettings.KEY_SCENE_MODE, CameraUtil.SCENE_MODE_NIGHT)
                    || notSame(hdrPref, CameraSettings.KEY_CAMERA_HDR, mSettingOff)) {
                buttonSetEnabled(mTsMakeupSwitcher, false);
            } else {
                buttonSetEnabled(mTsMakeupSwitcher, true);
            }
        }
    ...
    public void initFlashModeButton(View button) {
        ...
        button.setOnClickListener(new OnClickListener() {
            ...
            public void onClick(View v) {
            //wangshenxing
               if (pref.getValue().equalsIgnoreCase("auto")) {
                   pref.setValue("on");
               } else if (pref.getValue().equalsIgnoreCase("on")) {
                   pref.setValue("off");
               } else if (pref.getValue().equalsIgnoreCase("off")) {
                   pref.setValue("auto");
               }
               updateFlashModeIcon(pref);
               onSettingChanged(pref);
    ...
    public void updateFlashModeIcon(IconListPreference pref) {
        ...
        if (ind == -1)
            ind = 0;
        ((ImageView) mFlashModeSwitcher).setImageResource(thumbnails[ind]);
    }
    //add by wangshenxing for camera ui 2017/05/25 end
