...
import com.android.camera.ui.HorizontalselectedView;///:iphone style
...
public class CameraActivity extends Activity
        ...
        HorizontalselectedView.HorizontalModuleSwitchListener,///:iphone style
    ...
    private boolean mSecureCameraOnStop = false;
    ...
    private CameraOpenErrorCallback mCameraOpenErrorCallback =
			...
			Log.e(TAG, "onCameraDisabled cameraId:"+cameraId);
			...
			Log.e(TAG, "onDeviceOpenFailure cameraId:"+cameraId);
                    CameraUtil.showErrorAndFinish(CameraActivity.this,
			...
			Log.e(TAG, "onReconnectionFailure");
                    CameraUtil.showErrorAndFinish(CameraActivity.this,
                            R.string.cannot_connect_camera);
                }
			...
			Log.e(TAG, "onStartPreviewFailure cameraId:"+cameraId);
                    CameraUtil.showErrorAndFinish(CameraActivity.this,
                            R.string.cannot_connect_camera);
                }
            };
    ...
    private BroadcastReceiver mSDcardMountedReceiver = new BroadcastReceiver() {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            //modified by muxudong for ICE2-1086 start
            if (action.equals(Intent.ACTION_MEDIA_UNMOUNTED)) {
                Storage.setSaveSDCard(false);
                mCurrentModule.switchSavePathToPhone();
            }
    ...
    public void onCreate(Bundle state) {
        ...
        if (checkPermissions() || !mHasCriticalPermissions) {//进来camera时检测权限
        ...
        if (INTENT_ACTION_STILL_IMAGE_CAMERA_SECURE.equals(action)
                ...
                || intent.getComponent().getClassName().equals(GESTURE_CAMERA_NAME)) {//是否是进安全模式camera，比如手势滑动进camera
            mSecureCamera = true;
        } else {
        ...
        mSettingsManager = new SettingsManager(this);//初始化settingManager，第一个重要manager

        LayoutInflater inflater = getLayoutInflater();//通过inflater注入布局
        View rootLayout = inflater.inflate(R.layout.camera, null, false);//root父布局
        ...
        if (MediaStore.INTENT_ACTION_VIDEO_CAMERA.equals(getIntent().getAction())
            ...
            moduleIndex = ModuleSwitcher.VIDEO_MODULE_INDEX;//录像模式
        } else if (MediaStore.INTENT_ACTION_STILL_IMAGE_CAMERA.equals(getIntent().getAction())
                || MediaStore.INTENT_ACTION_STILL_IMAGE_CAMERA_SECURE.equals(getIntent()
                        .getAction())) {
            moduleIndex = ModuleSwitcher.PHOTO_MODULE_INDEX;//拍照模式
        ...
        boolean cam2on = mSettingsManager.isCamera2On();//双camera
        if (cam2on && moduleIndex == ModuleSwitcher.PHOTO_MODULE_INDEX)
            moduleIndex = ModuleSwitcher.CAPTURE_MODULE_INDEX;//双camera
        ...
        mMainHandler = new MainHandler(getMainLooper());//整个ui改变交互就靠他了

        mAboveFilmstripControlLayout =
                (FrameLayout) findViewById(R.id.camera_above_filmstrip_layout);
        mAboveFilmstripControlLayout.setFitsSystemWindows(true);//视图自适应和有无actionbar和navigationbar相关
        mPanoramaManager = AppManagerFactory.getInstance(this)
                .getPanoramaStitchingManager();//长图拍照模式管理
        ...
        if (!mSecureCamera) {//如果不是安全模式拍照正常打开图片
            ...
            ImageView v = (ImageView) getLayoutInflater().inflate(
                    R.layout.secure_album_placeholder, null);//有锁的图片
        ...
        if (requestCode == REQ_CODE_DONT_SWITCH_TO_PREVIEW) {
            ...
            if(resultCode == RESULT_OK) {
                byte[] jpeg = data.getByteArrayExtra("thumbnail");
                        Log.w(TAG, "hss66_0 onActivityResult");
                if(jpeg != null) {
                        Log.w(TAG, "hss66_1 onActivityResult");
                    updateThumbnail(jpeg);//更新缩略图
                }
            }
        } else {
    ...
    private boolean checkPermissions() {
        ...
        //modify by muxudong for ICE2-366 start
        if (INTENT_ACTION_STILL_IMAGE_CAMERA_SECURE.equals(getIntent().getAction())) {
            //do nothing
        } else if (!isRequestShown || !mHasCriticalPermissions) {
        ...
        //modify by muxudong for ICE2-366 end
    ...
    public void onResume() {
		...
		if (isInMultiWindowMode()){// add for ICE15-669 & ICE15-718
			...
			Toast.makeText(this, "Don't support split screen !", Toast.LENGTH_SHORT).show();
            finish();
            return;
		}
	...
	if(MediaStore.INTENT_ACTION_STILL_IMAGE_CAMERA_SECURE.equals(getIntent().getAction())){
		Log.d(TAG, "onStop "+mSecureCameraOnStop);
		if(mSecureCameraOnStop){
			finish();
		}else{
			mSecureCameraOnStop = true;
		}
	}
    }
    ...
    ///:iphone style start
    @Override
    public void onHorizontalModuleSelected(int moduleIndex) {//视图ui点击后调用这个实现来切换模式
        boolean cam2on = mSettingsManager.isCamera2On();
        mForceReleaseCamera = moduleIndex == ModuleSwitcher.CAPTURE_MODULE_INDEX ||
                (cam2on && moduleIndex == ModuleSwitcher.PHOTO_MODULE_INDEX);
        if (mForceReleaseCamera) {
