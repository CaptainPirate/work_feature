...
/**
 * Created by hss on 2017-7-27
 */

public class HorizontalselectedView extends View {
    private static final String TAG = "CAM_HorizontalselectedView";
    private static final int SWITCHER_POPUP_ANIM_DURATION = 200;

    public static final int PHOTO_MODULE_INDEX = 0;//如果支持双camera模式就使用CAPTURE_MODULE_INDEX否则使用PHOTO_MODULE_INDEX
    ...
    public static final int[] mModuleIds={
     		PHOTO_MODULE_INDEX,
		VIDEO_MODULE_INDEX,
                WIDE_ANGLE_PANO_MODULE_INDEX,
               /* LIGHTCYCLE_MODULE_INDEX,
                GCAM_MODULE_INDEX,
    ...
    private HorizontalModuleSwitchListener mListener;

    public interface HorizontalModuleSwitchListener {
        public void onHorizontalModuleSelected(int i);
    ...
    private void initAttrs(AttributeSet attrs) {
        ...
        //两种字体颜色和字体大小
        seeSize = tta.getInteger(R.styleable.HorizontalselectedView_HorizontalselectedViewSeesize, 6);
        selectedTextSize = tta.getFloat(R.styleable.HorizontalselectedView_HorizontalselectedViewSelectedTextSize, 35);
        selectedColor = tta.getColor(R.styleable.HorizontalselectedView_HorizontalselectedViewSelectedTextColor, context.getResources().getColor(R.color.iphone_style_select_textcolor));
        textSize = tta.getFloat(R.styleable.HorizontalselectedView_HorizontalselectedViewTextSize, 35);
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        switch (event.getAction()) {
            ...
            case MotionEvent.ACTION_UP:
                ...
                anOffset = 0;
                invalidate();
                String tag = getSelectedString();
                if(tag.equals("照片")){
                onHorizontalModuleSelected(0);
                }else if(tag.equals("视频")){
                onHorizontalModuleSelected(1);
                }else if(tag.equals("全景拍照")){
                onHorizontalModuleSelected(2);
                }else{
                onHorizontalModuleSelected(0);
                }
        Log.e(TAG, "onTouchEvent UP: n=" + n+"getSelectedString()="+getSelectedString());
                //onHorizontalModuleSelected(n);
                break;
            default:
                break;
        }
     ...
     public void setHorizontalSwitchListener(HorizontalModuleSwitchListener l) {
        mListener = l;
    }
     ...
     * 模式选择拍照，录像，全景
     **/
     private void onHorizontalModuleSelected(int ix) {
        //hidePopup();
        Log.e(TAG, "onHorizontalModuleSelected: ix= " + ix+"mCurrentIndex = "+mCurrentIndex);
        if (/*(ix != mCurrentIndex) && */(mListener != null)) {
        Log.e(TAG, "onHorizontalModuleSelected: come");
            ...
            //setCurrentIndex(ix);
            mCurrentIndex = ix;
            mListener.onHorizontalModuleSelected(mModuleIds[mCurrentIndex]);
        }
     }
