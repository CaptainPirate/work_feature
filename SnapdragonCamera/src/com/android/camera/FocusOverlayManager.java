...
public class FocusOverlayManager {
    ...
    public interface Listener {//在对应module里面实现，比如PhotoModule
    ...
    public FocusOverlayManager(ComboPreferences preferences, String[] defaultFocusModes,
        ...
        setParameters(parameters);//这个设置是找出支持的参数
        mListener = listener;
        setMirror(mirror);
        mUI = ui;
    }
    ...
    public void setParameters(Parameters parameters) {//得到的是全部参数，我们需要对参数进行重新设置，过滤出支持的参数
    ...
    public void doSnap() {//出发拍照走到这里调用capture()
        if (!mInitialized) return;
