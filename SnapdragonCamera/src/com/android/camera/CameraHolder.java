        ...
        if (mCameraDevice == null) {
            ...
            if (mMockCameraInfo == null) {
                mCameraDevice = CameraManagerFactory
                        .getAndroidCameraManager().cameraOpen(handler, cameraId, cb);//打开camera,在AndroidCameraManagerImpl中调用framework接口
            } else {
                if (mMockCamera != null) {
            ...
            mCameraId = cameraId;
            mParameters = mCameraDevice.getCamera().getParameters();//获取camera配置参数
        } else {
            if (!mCameraDevice.reconnect(handler, cb)) {//这里一般是在上面mCameraId == cameraId时调用，比如按home健后重进camera，只要应用还在后台
                Log.e(TAG, "fail to reconnect Camera:" + mCameraId + ", aborting.");
                return null;
            }
            mCameraDevice.setParameters(mParameters);//将缓存的参数设置回去
        }
        mCameraOpened = true;
