...
class AndroidCameraManagerImpl implements CameraManager {
    ...
    private ConditionVariable mSig = new ConditionVariable();//用于线程同步。提供了三个方法block()、open()、close()
        ...
        public void handleMessage(final Message msg) {
            try {
                    ...
                    case OPEN_CAMERA:
                        try {//通过反射调用camera
                            Method openMethod = Class.forName("android.hardware.Camera").getMethod(
                                    "openLegacy", int.class, int.class);//通过反射找到framework/base/core/java/android/hardware/camera里面的openLegacy方法
                            mCamera = (android.hardware.Camera) openMethod.invoke(
                                    null, msg.arg1, CAMERA_HAL_API_VERSION_1_0);//选择camera的版本CAMERA_HAL_API_VERSION_1_0
                        } catch (Exception e) {
                            /* Retry with open if openLegacy doesn't exist/fails */
                            Log.v(TAG, "openLegacy failed due to " + e.getMessage()
                                    + ", using open instead");
                            mCamera = android.hardware.Camera.open(msg.arg1);//一般方式去打开camera
                        }
                ...
                if (msg.what != RELEASE && mCamera != null) {
                    ...
                    mCamera = null;
                    ///M: ray add for face detection when google photos try to connect camera  ICE15-1711,1714 {@
                    if (msg.what == STOP_FACE_DETECTION)  {
                        Log.e(TAG, "Fail to stop face detection.");
                        return;
                    }///@}
    ...
    public CameraManager.CameraProxy cameraOpen(
        ...
        mCameraHandler.obtainMessage(OPEN_CAMERA, cameraId, 0, mCameraHandler.errorCbInstance)
                .sendToTarget();
        mCameraHandler.waitDone();//线程同步，保证mCameraHandler完成再去走下面的代码，这种等待容易anr的
        if (mCamera != null) {
    ...
    public class AndroidCameraProxyImpl implements CameraManager.CameraProxy {
        ...
        private AndroidCameraProxyImpl() {
            Assert(mCamera != null);//断言如果没有得到camera就抛出camera异常
        }
        ...
        @Override
        public boolean reconnect(Handler handler, CameraOpenErrorCallback cb) {//重连camera，在cameraHolder里面调用
        ...
        public void setParameters(Parameters params) {
            ...
            mSig.close();//线程同步close()————处理的线程 --block()，达到条件>open()，确保使用camera时已经完成参数的设置
        ...
        public Parameters getParameters() {
            ...
            mCameraHandler.waitDone();//等到上面的条件达成就可以往下走了
            return mParameters;
        }
