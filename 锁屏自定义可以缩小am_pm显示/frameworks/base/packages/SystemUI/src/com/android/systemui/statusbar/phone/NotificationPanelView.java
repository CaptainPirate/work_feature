...
import com.transsion.systemui.mgzkeyguard.widgets.SettingsUtil;

///:M[new clock] Modify--@{
import com.android.keyguard.clock.ClockView;
///:--@}
public class NotificationPanelView extends PanelView implements
    ...
    ///M:[HQ_3RD_MGZ_KEYGURAD]Add by xlq start{@
    private KeyguardBottomAreaView mKeyguardBottomArea;
    private static final String ACTION_HANDLE_KEYGUARD_BOTTOM_VIEW = "action.handle.keyguardbottomview";
    private static final String INTENT_EXTRA = "isHideLeftRightIcon";
    private HandleKeyguardBottomViewBroadcast mHandleKeyguardBottomViewBroadcast;
    private IntentFilter mIntentFilter;
    ///:M[new clock] Modify--@{
    private ClockView mNewClockView;
    ///:--@}
    class HandleKeyguardBottomViewBroadcast extends BroadcastReceiver {
	@Override
        public void onReceive(Context context, Intent intent) {
            final String action = intent.getAction();
            boolean isHideLeftRightIcon = intent.getBooleanExtra(INTENT_EXTRA, false);
            if (action.equals(ACTION_HANDLE_KEYGUARD_BOTTOM_VIEW)){
                if (!mKeyguardBottomArea.isCameraDisable()){
                    if (isHideLeftRightIcon){
                        getLeftIcon().setVisibility(View.INVISIBLE);
                        getRightIcon().setVisibility(View.INVISIBLE);
                    }else {
                        getLeftIcon().setVisibility(View.VISIBLE);
	...
	///M:[HQ_3RD_MGZ_KEYGURAD]Add by xlq start{@
	mKeyguardBottomArea = new KeyguardBottomAreaView(context,attrs);
        mIntentFilter = new IntentFilter();
        mIntentFilter.addAction(ACTION_HANDLE_KEYGUARD_BOTTOM_VIEW);
	///@}
    }
    ...
    protected void onFinishInflate() {
        ...
        if (false) {
            ...
            mNewClockView = (ClockView) findViewById(R.id.clock_view);
        }
        ///:M--@}
    ...
    public void onAttachedToWindow() {
        mContext.registerReceiver(mHandleKeyguardBottomViewBroadcast, mIntentFilter);
        super.onAttachedToWindow();
    }

    @Override
    public void onDetachedFromWindow() {
        mContext.unregisterReceiver(mHandleKeyguardBottomViewBroadcast);
    ...
    protected void onLayout(boolean changed, int left, int top, int right, int bottom) {
        ...
        if (false) {
            ...
            mKeyguardStatusView.setPivotY(
                    (FONT_HEIGHT - CAP_HEIGHT) / 2048f * mNewClockView.getHeight());
        }
        ///:--@}
	...
	///M:[HQ_3RD_MGZ_KEYGURAD]Add by xlq start{@
	int keyguardStatusHeight = 0;
	boolean isMgzKeyguardOn = false;
	isMgzKeyguardOn = SettingsUtil.isMgzKeyguardOn(mContext);
	if (isMgzKeyguardOn) {
	    keyguardStatusHeight = 0;
	} else if (mKeyguardStatusView != null){
	    keyguardStatusHeight = mKeyguardStatusView.getHeight();
	}
	    ...
	    mClockPositionAlgorithm.run(mClockPositionResult,isMgzKeyguardOn);
	   ///@}
    ...
    private void setQsExpanded(boolean expanded) {
        /// M: [HQ_MONKEY_TEST] add for monkey test @{
    ...
    private void setKeyguardStatusViewVisibility(int statusBarState, boolean keyguardFadingAway,
        ...
        if ((!keyguardFadingAway && mStatusBarState == StatusBarState.KEYGUARD
            ...
            mKeyguardStatusView.animate().cancel();
	    ///M:[HQ_3RD_MGZ_KEYGURAD]Add by xlq start{@
	    if (SettingsUtil.isMgzKeyguardOn(mContext)){
		mKeyguardStatusView.setVisibility(View.GONE);
	    } else {
    ...
    private void setQsExpansion(float height) {
        ...
        ///[HQ_INFINIX_NEW_UI] @{
        if ((mStatusBarState == StatusBarState.KEYGUARD || mStatusBarState == StatusBarState.SHADE_LOCKED)){
            android.util.Log.d("BingWu.Lee","[mStatusBarState] = "+mStatusBarState+"  [height] = "+height + " [isBlurInited] = "+isBlurInited);
            if (height >= 360) {
                if (!isBlurInited) {
                    mStatusBar.setWaterMask(false);
                    isBlurInited = true;
                } else {
                    mStatusBar.setWaterMask(true);
                }
            } else {
                mStatusBar.setRollbackground();
            }
        }
        ///@}
    ...
    protected void onHeightUpdated(float expandedHeight) {
        ///[HQ_INFINIX_NEW_UI] @{
        if(mQsExpanded && !mQsExpandImmediate && !mIsExpanding && !mQsExpandedWhenExpandingStarted && mStatusBarState== StatusBarState.KEYGUARD){
            mKeyguardStatusView.setVisibility(View.GONE);
        }
        if(!mQsExpanded && !mQsExpandImmediate && !mIsExpanding && !mQsExpandedWhenExpandingStarted && mStatusBarState== StatusBarState.KEYGUARD){
            //mKeyguardStatusView.setVisibility(View.VISIBLE);
    ...
    private float getKeyguardContentsAlpha() {
        ...
        alpha = (float) Math.pow(alpha, 0.75);
	///M:[HQ_3RD_MGZ_KEYGURAD]Add by xlq start{@
	if (SettingsUtil.isMgzKeyguardOn(mContext) && alpha != 0.0f){
	    alpha = 1.0f;
	}
	///@}
        return alpha;
